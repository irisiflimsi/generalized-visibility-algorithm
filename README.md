...
### G
All obstacles at $p_i$ are to the left of our current axis and we are not at an outer left vertex $c_i$ as viewed from $p_i$. Rotating $CCW$, we first encounter an obstacle from $P$ connected to $q_i$. Note that $q_{i+1}$ will not coincide with $p_i$, because we would have been in position (P) before.
...
Take any point $q\in V(C)$ and take a line-of-sight to $c$ on a boundary segment $(c^E_0,c^E_1)$. By definition, there is no obstacle $p$ on the line-of-sight, so we rotate $c$ in $CCW$ direction, until we either hit an outer left vertex $c^E_k$ of $C$ or some $p_k$ in $P$, which then has no other endpoint to the right. If we don't have $p_k$ with this non-right property yet, rotate around $c$ in $CW$ until we do. Note that the whole triangle $(p_k, c, q)$ is visible from $c$, although it may be a straight line segment. See position (X). 

By nature of the algorithm, we will touch all obstacle points as we go around $C$. Not all obstacle points will be rotation points (or a $q$ for triangle completion). Our algorithm will be at point $c^E_0$ at some point and move along the boundary across $c$, possibly multiple times. Let assume Let $c_i$ and $p_i$ be the points where $p_k$ is on the ray $(c_i,p_i)$ beyond $p_i$. 
